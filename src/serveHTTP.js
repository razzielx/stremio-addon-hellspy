...
function serveHTTP(addonInterface, opts = {}) {
  ...
  if (cacheMaxAge > 365 * 24 * 60 * 60) console.warn("cacheMaxAge set to more then 1 year, be advised that cache times are in seconds, not milliseconds.");

  const app = express();
  app.use((_, res, next) => {
    if (cacheMaxAge && !res.getHeader("Cache-Control")) res.setHeader("Cache-Control", "max-age=" + cacheMaxAge + ", public");
    next();
  });
